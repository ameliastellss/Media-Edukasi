...
<body>
  ...
  <div class="hero_area">
    ...
    <!-- end header section -->
<body>
  <h3>LATIHAN SOAL BAB magnet APPLICATION LETTER </h3>

  <table border="1">
    <tbody>
      <?php
        include "koneksi_magnet.php";
        $sql = "SELECT * FROM soal_magnet WHERE aktif='Y' ORDER BY RAND ()";
        $query = mysqli_query($koneksi,$sql) or die (mysqli_error($koneksi));
        $jumlah = mysqli_num_rows($query);
        $no = 0;
        while($data = mysqli_fetch_array($query)){?>
          <form action="kunci_magnet.php" method="post">
            ...
            <?php
              if(!empty($data['gambar'])){
                echo "<tr><td></td><td> <img src= $data[gambar]' width='200' height='200'></td></tr>";
