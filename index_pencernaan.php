...
<body>
  ...
  <div class="hero_area">
    ...
    <!-- end header section -->
<body>
  <h3>LATIHAN SOAL BAB pencernaan LAMARAN</h3>

  <table border="1">
    <tbody>
      <?php
        include "koneksi_pencernaan.php";
        $sql = "SELECT * FROM soal_pencernaan WHERE aktif='Y' ORDER BY RAND ()";
        $query = mysqli_query($koneksi,$sql) or die (mysqli_error($koneksi));
        $jumlah = mysqli_num_rows($query);
        $no = 0;
        while($data = mysqli_fetch_array($query)){?>
          <form action="kunci_pencernaan.php" method="post">
            ...
            <tr>
              <td></td>
              <td>D. <input name="pilihan[<?php echo $data['id_soal']?>]" type="radio" value="D"><?php echo $data['C'];?></td>
            </tr>
      ...
      ?>
        ...
        <tr>
          <td></td>
          <td>
            <input type="submit" name="submit" value="Submit" onclick="return confirm('Apakah Anda yakin dengan jawaban Anda?')">
          </td>
        </tr>
