...
<body>
  ...
  <div class="hero_area">
    ...
    <!-- end header section -->
<body>
  <h3>LATIHAN SOAL BAB sel SISTEM PEMBELAHAN </h3>

  <table border="1">
    <tbody>
      <?php
        include "koneksi_sel.php";
        $sql = "SELECT * FROM soal_sel WHERE aktif='Y' ORDER BY RAND ()";
        $query = mysqli_query($koneksi,$sql) or die (mysqli_error($koneksi));
        $jumlah = mysqli_num_rows($query);
        $no = 0;
        while($data = mysqli_fetch_array($query)){?>
          <form action="kunci_sel.php" method="post">
